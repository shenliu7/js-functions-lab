...
Complete the exercise in the space below:
*/
const generateEmail = (aName, aDomain) => {
    return aName + "@" + aDomain
}
...
const maxOfThree = (numA, numB, numC) => {
    if (numA - numB > 0 && numA- numC >0) {
        return numA;
    } else if (numB - numA > 0 && numB - numC > 0) {
        return numB;
    } else if (numC - numA > 0 && numC - numA > 0) {
        return numC;
    }
}
...
Complete the exercise in the space below:
*/
const calculateTip = (total, perctange) => {
    return total * perctange / 100;
}
